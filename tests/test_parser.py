...
def test_parse_line_parses_matching_line_and_extracts_variables(
    ...
    assert (
        parsed["template"]
        == "Signal '<VAR>' float Signal '<VAR>' float '<VAR>' signal conflicted"
    )


def test_parse_line_matching_prefix_with_no_quotes_yields_no_var(parser):
    line = "1 of 1 0 no_quoted_variables_here"
...
def test_parse_line_rejects_counter_in_middle_of_prose(parser):
    line = "prefix text 1 of 2 0 Signal 'u_top' not found"
